...
The CUE module lives under `src/` — both the catalog package files and `cue.mod/` sit there, so `src/` is the module root and the import path stays `opmodel.dev/catalogs/opm@v2` (no per-version subdirectory). The generated definition index ships inside `src/` so it travels with the published module; everything else (README, Taskfile, CI workflows) stays at the repo root.
