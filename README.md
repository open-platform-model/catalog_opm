...
This repository is a single CUE module, `opmodel.dev/catalogs/opm@v0`, published to `ghcr.io/open-platform-model/catalogs/opm` and consumed via `import "opmodel.dev/catalogs/opm@v0"` (package `opm`).
...
The CUE module lives under `src/` — both the catalog package files and `cue.mod/` sit there, so `src/` is the module root and the import path stays `opmodel.dev/catalogs/opm@v0` (no per-version subdirectory). The generated definition index ships inside `src/` so it travels with the published module; everything else (README, Taskfile, CI workflows) stays at the repo root.
